...
(function() {
    ...
    function initialize(customConfig = {}) {
        ...
        if (config.debug) {
            console.log('Novu Session Interceptor initialized with TTL:', config.ttl, 'ms');
        }
    }
    ...
    function storeSessionData(data) {
        ...
        logDebug('Session data stored with expiry at:', new Date(sessionData.expiry).toLocaleString());
    }
    ...
    function getSessionData() {
        ...
        if (now > sessionData.expiry) {
            logDebug('Stored session data expired at:', new Date(sessionData.expiry).toLocaleString());
            return null;
        }
        
        logDebug('Retrieved valid session data, expires in:', 
            Math.round((sessionData.expiry - now) / 1000 / 60), 'minutes');
    ...
    function setupTokenInterception() {
        ...
        window.fetch = function(url, options) {
            ...
            if (typeof url === 'string' && url.includes('api.novu.co/v1/inbox/session')) {
                // Try to get cached data first
                const cachedData = getSessionData();
                if (cachedData) {
                    logDebug('Returning cached session data instead of making API call');
                ...
                promise.then(response => {
                    ...
                    if (response.ok) {
                        ...
                        clone.json().then(data => {
                            ...
                            console.error('Error parsing Novu session response:', err);
                        });
                    }
                }).catch(err => {
